...
export default function Home() {
  ...
  const handleSubmit = async (e) => {
    ...
    setFormData({ project_name: "", kva: "", qty: "", eng: "", region: "", recd: "", month: "", start_date: "", end_date: "" });
    setEditingId(null);
    fetchCompleted();
  };
            ...
            {Object.entries(formData).map(([key, value]) => (
              key !== 'id' && (
                <div key={key}>
                  ...
                  <input
                    type={key.includes('date') ? 'date' : key === 'kva' || key === 'qty' ? 'number' : 'text'}
                    name={key}
                    value={value}
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-50">
                <tr>
                  {['SR', 'Project', 'KVA', 'Qty', 'Engineer', 'Region', 'Recd', 'Month', 'Start', 'End', 'Actions'].map((header) => (
                    <th
                      key={header}
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredProjects.map((record, index) => (
                  <tr key={record.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{record.kva}</td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{record.start_date}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{record.end_date}</td>
